...
namespace SC
{
	...
	public:
		...
		void ToArray(ArrayList<T> &dst) const
		{
			AddNodeToArray(dst, m_head);
		}

		void AddNodeToArray(ArrayList<T> &dst, Node *head) const
		{
			if (head == nullptr)
				return;
			dst.Add(head->pair.value);

			AddNodeToArray(dst, head->a);
			AddNodeToArray(dst, head->b);
		}
